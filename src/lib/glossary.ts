...
/* ───────────────────── 상태 2축 (기획 5) — 단일 원천 ─────────────────────
 *
 * responses.status 가 원천이고 participants.account_status 7종은 트리거가 동기화한다.
 * 화면은 원상태 7종을 그대로 나열하지 않고 두 축으로 접어 보여 준다.
 *   진행축 — 참여자가 어디까지 왔나 (4단)
 *   검토축 — 제출된 응답이 어떤 판정 상태인가 (제출 이후에만 존재)
 */

/** 진행축 — 원상태(계정 상태 7종) → 진행 단계 4단. */
export const PROGRESS_OF: Record<string, string> = {
  미발송: "미발송",
  초대발송: "미확인",
  미접속: "미확인",
  작성중: "작성중",
...
/** 진행축의 표시 순서. */
export const PROGRESS_ORDER = ["미발송", "미확인", "작성중", "제출"] as const;

/** 검토축 — 여기 없는 원상태는 검토축이 없다(null). */
export const REVIEW_OF: Record<string, string> = {
  제출: "검토 대기",
  반려: "반려",
  승인: "승인",
};
